...
public class ETLMoh731BCohortLibrary {
    ...
    public CohortDefinition activeFsw(){
        ...
        String sqlQuery ="select  en.client_id  from kp_etl.etl_client_enrollment en  inner join openmrs.encounter e on e.patient_id = en.client_id\n" +
                "inner join kp_etl.etl_client_social_status s on en.client_id = s.client_id\n" +
    ...
    public CohortDefinition activePwud(){
        ...
        String sqlQuery ="select  en.client_id  from kp_etl.etl_client_enrollment en  inner join openmrs.encounter e on e.patient_id = en.client_id\n" +
                ...
                "group by en.client_id\n" +
                "having timestampdiff(MONTH,Max(date(e.encounter_datetime)),date(:endDate))<=3;                      inner join kp_etl.etl_client_social_status s on en.client_id = s.client_id\n" +
                "where s.key_population_type = \"People who use drugs\"  and en.voided = 0\n" +
                "group by en.client_id\n" +
                "having timestampdiff(MONTH,Max(date(e.encounter_datetime)),date(:endDate))<=3;";
    ...
    public CohortDefinition hivTestedFsw(){
        ...
        String sqlQuery ="select h.client_id from kp_etl.etl_hts_test h inner join kp_etl.etl_client_enrollment e on h.client_id = e.client_id\n" +
                "inner join kp_etl.etl_client_social_status s on h.client_id = s.client_id\n" +
...
//knownPositiveFsw
    public CohortDefinition knownPositiveFsw(){
        ...
        String sqlQuery ="select h.client_id from kp_etl.etl_hts_test h inner join kp_etl.etl_client_enrollment e on h.client_id = e.client_id\n" +
                "                                              inner join kp_etl.etl_client_social_status s on h.client_id = s.client_id\n" +
                "inner join openmrs.encounter en on h.client_id = en.patient_id\n" +
...
//linkedFsw
    public CohortDefinition linkedFsw(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="\n" +
                "select k.client_id from ((select t.client_id from etl_hts_test t inner join etl_hts_referral_and_linkage r on t.client_id=r.client_id where  timestampdiff(MONTH,date(t.visit_date),date(:enddate))<=3 and t.final_test_result=\"Positive\")\n" +
                "union all (select hs.client_id from etl_hiv_status hs\n" +
                "                       where hs.current_in_care =\"Yes\" and timestampdiff(MONTH,date(hs.test_date),date(:enddate))<=3 ))k  inner join etl_client_social_status ss on ss.client_id = k.client_id\n" +
...
//linkedMsm
    public CohortDefinition linkedMsm(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select k.client_id from ((select t.client_id from etl_hts_test t inner join etl_hts_referral_and_linkage r on t.client_id=r.client_id where  timestampdiff(MONTH,date(t.visit_date),date(:enddate))<=3 and t.final_test_result=\"Positive\")\n" +
                "                         union all (select hs.client_id from etl_hiv_status hs\n" +
                "                                    where hs.current_in_care =\"Yes\" and timestampdiff(MONTH,date(hs.test_date),date(:enddate))<=3 ))k  inner join etl_client_social_status ss on ss.client_id = k.client_id\n" +
...
/*screenedForSTIFsw*/
    public CohortDefinition screenedForSTIFsw(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select sc.client_id from etl_sti_screening sc inner join etl_client_social_status ss on ss.client_id = sc.client_id\n" +
                "where sc.sti_screening_done = \"Yes\" and ss.key_population_type = \"Female sex worker\"  and sc.voided = 0;";
...
//screenedForSTIMsm
    public CohortDefinition screenedForSTIMsm(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select sc.client_id from etl_sti_screening sc inner join etl_client_social_status ss on ss.client_id = sc.client_id\n" +
                "where sc.sti_screening_done = \"Yes\" and ss.key_population_type = \"Male who have sex with Men\"  and sc.voided = 0;";
...
//screenedForSTIMsw
    public CohortDefinition screenedForSTIMsw(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select sc.client_id from etl_sti_screening sc inner join etl_client_social_status ss on ss.client_id = sc.client_id\n" +
                "where sc.sti_screening_done = \"Yes\" and ss.key_population_type = \"Male sex worker\"  and sc.voided = 0;";
    ...
    public CohortDefinition screenedForSTIPwid(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select sc.client_id from etl_sti_screening sc inner join etl_client_social_status ss on ss.client_id = sc.client_id\n" +
                "where sc.sti_screening_done = \"Yes\" and ss.key_population_type = \"People who inject drugs\" and sc.voided = 0;";
    ...
    public CohortDefinition screenedForSTIPwud(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select sc.client_id from etl_sti_screening sc inner join etl_client_social_status ss on ss.client_id = sc.client_id\n" +
                "where sc.sti_screening_done = \"Yes\" and ss.key_population_type = \"People who use drugs\" and sc.voided = 0;";
    ...
    public CohortDefinition screenedForHCVFsw(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select hp.client_id from etl_hepatitis_screening hp inner join etl_client_social_status ss on ss.client_id = hp.client_id\n" +
                "where hp.hepatitis_screening_done = \"Hepatitis C\" and ss.key_population_type = \"Female sex worker\" and hp.voided = 0;";
...
//screenedForHCVMsm
    public CohortDefinition screenedForHCVMsm(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select hp.client_id from etl_hepatitis_screening hp inner join etl_client_social_status ss on ss.client_id = hp.client_id\n" +
                "where hp.hepatitis_screening_done = \"Hepatitis C\" and ss.key_population_type = \"Male who have sex with Men\" and hp.voided = 0;";
    ...
    public CohortDefinition screenedForHCVMsw(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select hp.client_id from etl_hepatitis_screening hp inner join etl_client_social_status ss on ss.client_id = hp.client_id\n" +
                "where hp.hepatitis_screening_done = \"Hepatitis C\" and ss.key_population_type = \"Male sex worker\" and hp.voided = 0;";
...
//screenedForHCVPwid
    public CohortDefinition screenedForHCVPwid(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select hp.client_id from etl_hepatitis_screening hp inner join etl_client_social_status ss on ss.client_id = hp.client_id\n" +
                "where hp.hepatitis_screening_done = \"Hepatitis C\" and ss.key_population_type = \"People who inject drugs\" and hp.voided = 0;";
...
//screenedForHCVPwud
    public CohortDefinition screenedForHCVPwud(){
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery ="select hp.client_id from etl_hepatitis_screening hp inner join etl_client_social_status ss on ss.client_id = hp.client_id\n" +
                "where hp.hepatitis_screening_done = \"Hepatitis C\" and ss.key_population_type = \"People who use drugs\" and hp.voided = 0;";
